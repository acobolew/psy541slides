---
title: "Class 3"
authors: "Tobias Gerstenberg, Alan Cobo-Lewis"
...
```{r, eval=FALSE}
install.packages(c("gganimate", "gapminder", "ggridges", "devtools", "png", "gifski", "patchwork", "ggforce", "ggstats"))
```
...
library("ggstats")   # for bar plots and their labels
```
...
And let's load the diamonds data again. (using `<-` rather than `=` -- but let's talk about that)
...
There are two things to learn from the use of `coord_polar()` here:

1.  Changing the coordinate system is something you can do in ggplot and will sometimes have to do.
2.  It's a pretty awkward way to make a pie chart--which helps teach the lesson of YOU SHOULD AVOID PIE CHARTS!
...
Boxplots are useful, but they're not flawless. Many data sets are consistent with the same boxplot.
...
If you do have a lot of data points, then violin plots can be helpful – and you can still superimpose boxplots on the violin plots.

If there is not too much data, you can plot jitterred individual data points instead (jitterred to avoid overplotting). Even better, you can use sina plots, which jitter the data to the width that the violin plot would be.
...
Here's how to add boxplots over a violin plot:
...
ggplot(data = df.diamonds,
       mapping = aes(x = color,
                     y = price)) +
  geom_violin() +
  geom_boxplot(width=.1, outlier.shape=NA, coef=0, fill='lightblue')
```

(You can find the advice about `outlier.shape=NA` and `coef=0` by googling *geom_boxplot no whiskers*, which sends you to <https://stackoverflow.com/questions/18459287/remove-whiskers-in-box-whisker-plot>)

It's important to draw the boxplot *after* the violin plot, because otherwise the violin plot will be drawn on top of it and hide it:
...
The df.diamonds dataset is large (`df.diamonds |> count()` rows!) so plotting individual data points, as in sina plot (from the `ggforce` package) would be silly. But let's take less than 1% of the dataset just to illustrate how a sina plot would work:

```{r}
df.diamonds |> count()

ggplot(data = df.diamonds |> sample_n(500),
       mapping = aes(x = color,
                     y = price)) +
  geom_violin() +
  geom_sina(size=1) +
  geom_boxplot(width=.1, outlier.shape=NA, coef=0, fill='lightblue', alpha=.8)
```

Using the sina plot exposes the details of the dataset and the 'big picture' of a dataset at the same time.
...
(Note the message `Warning: Using size aesthetic for lines was deprecated in ggplot2 3.4.0. Please use linewidth instead.`)
...
ggplot(data = df.diamonds,
  ...
  stat_summary(fun = "mean",
               ...
               size = 2) +
  theme(legend.position = "bottom",
        legend.direction = 'vertical') + # so legend title at top not side
...
viridis is very colorblind-safe as in `scale_fill_viridis_d` and `scale_color_viridis_d`. These are now a part of the `ggplot2` package.

You can also check plots with [CVSimulator](https://asada.website/cvsimulator/e/) (available for iphone, android, and on the web). I use this!

Another trick is to redundantly code, so that information conveyed with color is also conveyed via another dimension–like using *shape* of a data point as well as color. This example isn't ideal (see the warning), but it should give you the idea:
...
ggplot(data = df.diamonds,
  ...
  guides(color = guide_legend(nrow = 3, # 3 rows 
         ...
         shape = guide_legend(nrow=3, reverse=TRUE, override.aes=list(size=6)))
```
...
-   [CVSimulator](https://asada.website/cvsimulator/e/) --\> for testing colorblind-friendliness
